...
def upload_image(file_bytes: bytes, filename: str, content_type: str) -> str:
    ...
    blob = bucket.blob(f"meal-images/{uuid.uuid4().hex}.{ext}")
...
def render_meal_card(db, day, person, idx, meal):
    ...
    if st.session_state.get(f"open_{doc_id}", False):
        new_comment = st.text_area("Note", value=comment, key=f"ta_{doc_id}",
                                   placeholder="Note or substitution…",
                                   label_visibility="collapsed", height=72)
        uploaded = st.file_uploader("Photo", type=["jpg","jpeg","png","heic"],
...
def render_snacks(db, day, person):
    ...
    for snack in load_snacks(db, day, person):
        ...
        st.markdown(f'<div class="snack-box">🍽 <b>{snack["desc"]}</b>'
    ...
    if st.session_state.get(add_key, False):
        desc = st.text_input("What did you have?", key=f"desc_{add_key}",
        ...
        if st.button("Log", key=f"log_{add_key}", use_container_width=True):
            if desc.strip():
                url = upload_image(img.read(), img.name, img.type) if img else ""
                add_snack(db, day, person, desc.strip(), url)
...
def main():
    ...
    selected_day = st.radio("Day", DAYS, index=default_idx,
